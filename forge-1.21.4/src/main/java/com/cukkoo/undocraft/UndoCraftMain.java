...
public class UndoCraftMain {
    ...
    public UndoCraftMain() {
        undoChannel = ChannelBuilder.named(ResourceLocation.fromNamespaceAndPath("undocraft", "undo"))
        ...
        hasHistoryChannel = ChannelBuilder.named(ResourceLocation.fromNamespaceAndPath("undocraft", "has_history"))
                .networkProtocolVersion(1)
                .simpleChannel();
    ...
    public static void syncHistoryState(ServerPlayer player) {
        if (player != null) {
            hasHistoryChannel.send(new HasHistoryPayload(CraftingHistory.hasHistory(player.getUUID())),
